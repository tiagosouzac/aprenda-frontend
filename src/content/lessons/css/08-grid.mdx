...
quiz:
  - question: "O que a declaração grid-template-columns: repeat(3, 1fr) cria?"
    options:
      ...
      - "Três linhas de altura igual"
      - "Uma coluna com três trilhas"
      - "Um grid de nove células"
    correct: 0
  - question: "O que é a unidade fr em CSS Grid?"
    options:
      - "Um pixel fracionário"
      - "A frequência de atualização do grid"
      - "Uma fração do espaço restante após descontar itens de tamanho fixo"
      - "Um alias para flex: 1"
    correct: 2
  - question: "Qual é a principal diferença entre Grid e Flexbox?"
    options:
      - "Grid usa JavaScript; Flexbox usa CSS puro"
      - "Grid é mais rápido que Flexbox"
      - "Flexbox suporta mais navegadores que Grid"
  ...
  - question: "O que acontece com um grid item que não cabe nas colunas definidas por grid-template-columns?"
    options:
      - "Ele é removido do layout"
      - "Ele vai para o grid implícito — linhas criadas automaticamente com tamanho definido por grid-auto-rows"
      - "Ele cria uma nova coluna automática"
      - "Ele herda o tamanho da última coluna definida"
    correct: 1
---
