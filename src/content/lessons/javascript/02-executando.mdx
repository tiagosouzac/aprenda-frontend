...
quiz:
  - question: "Qual é a diferença entre defer e async em uma tag script?"
    options:
      - "defer é para scripts externos; async é para scripts inline"
  ...
  - question: "Por que scripts colocados no head sem defer ou async bloqueiam a renderização?"
    options:
      - "Porque o CSS precisa carregar primeiro"
      - "Porque o navegador espera o DOM estar completo antes de executar qualquer script"
      - "Porque head não suporta execução de código"
  ...
  - question: "O que type='module' em uma tag script habilita?"
    options:
      ...
      - "Execução de código TypeScript diretamente"
      - "Carregamento de arquivos CSS pelo JavaScript"
    correct: 1
  - question: "Por que onclick='...' diretamente no HTML deve ser evitado?"
    options:
      ...
      - "Porque onclick não funciona em todos os navegadores"
      - "Porque onclick executa mais lentamente que addEventListener"
      - "Porque onClick (com maiúscula) é a forma correta"
    correct: 0
---
