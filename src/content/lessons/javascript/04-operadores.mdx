...
quiz:
  - question: "Qual é a diferença entre || e ?? (nullish coalescing)?"
    options:
      - "O operador || retorna o operando da direita apenas quando o da esquerda é null ou undefined, e o ?? faz isso para qualquer valor falsy"
      - "Os dois operadores são sinônimos e fazem exatamente a mesma coisa, então tanto faz qual deles você escolhe usar no seu código"
      - "?? retorna o operando direito se o esquerdo for null ou undefined; || retorna se for qualquer valor falsy — incluindo 0, '' e false"
      - "O operador || foi projetado para funcionar apenas com valores booleanos verdadeiros ou falsos, enquanto o ?? foi pensado para lidar com objetos"
    correct: 2
  - question: "O que é curto-circuito em operadores lógicos?"
    options:
      - "É quando o uso de um operador lógico acaba provocando um erro de execução que interrompe o programa naquele exato ponto"
      - "É quando dois operadores lógicos diferentes são usados juntos dentro de uma mesma expressão um pouco mais complexa"
      - "É uma otimização interna do motor JavaScript que arredonda os números automaticamente para evitar erros de cálculo com ponto flutuante"
  ...
  - question: "O que o operador % (módulo) retorna?"
    options:
      - "O resultado completo da divisão entre os dois números, incluindo as casas decimais que sobram"
      - "O resto que sobra de uma divisão inteira entre o primeiro e o segundo número"
      - "O valor percentual que um número representa em relação ao outro número da conta"
      - "A diferença numérica entre o primeiro operando e o segundo operando da expressão"
    correct: 1
  - question: "Quando usar o operador ternário em vez de if/else?"
    options:
      ...
      - "Sempre que for possível, porque o operador ternário é executado de forma mais eficiente do que um bloco if/else completo"
      - "Nunca, porque um bloco if/else é sempre mais legível e o ternário só serve para confundir quem vai ler o código depois"
      - "Apenas nas situações em que a condição que está sendo avaliada envolve diretamente os valores null ou undefined"
    correct: 0
---
