...
quiz:
  - question: "Qual é a diferença entre map() e filter()?"
    options:
      - "O map() modifica diretamente o array original durante a iteração, enquanto o filter() é o único dos dois que cria um array novo"
      - "O map() foi feito para trabalhar com arrays de números, enquanto o filter() foi pensado para trabalhar com arrays de strings"
      ...
      - "Não existe diferença entre os dois: são apenas sinônimos com nomes diferentes que fazem exatamente a mesma transformação"
    correct: 2
  - question: "Por que preferir [...array, novoItem] a array.push(novoItem)?"
    options:
      - "O operador spread é executado de forma perceptivelmente mais rápida pelo motor JavaScript do que o método push()"
      - "O método push() simplesmente não funciona nos arrays que são criados pela sintaxe moderna do JavaScript usada hoje"
      - "O uso do spread é uma exigência técnica imposta pelo React, que não aceita arrays criados ou alterados com push()"
  ...
  - question: "O que reduce() faz?"
    options:
      - "Reduz o tamanho do array removendo dele todos os itens que estiverem duplicados, deixando apenas valores únicos"
      - "Acumula todos os itens em um único valor — passando o resultado de cada iteração para a próxima"
      - "Percorre o array inteiro e remove dele todos os itens cujo valor seja considerado falsy pela linguagem JavaScript"
      - "Percorre todos os itens do array e retorna apenas aquele que tiver o menor valor numérico entre todos eles"
    correct: 1
  - question: "Qual é a diferença entre find() e filter()?"
    options:
      ...
      - "Não existe diferença prática entre find() e filter(): os dois percorrem o array e devolvem o mesmo tipo de resultado"
      - "O método find() é executado de forma perceptivelmente mais rápida que o filter() ao percorrer um array grande na memória"
      - "O filter() é que retorna apenas um único item do array, enquanto o find() retorna todos os itens que passam no teste"
    correct: 0
---
