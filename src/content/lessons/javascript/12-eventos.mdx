...
quiz:
  - question: "Qual é a diferença entre event.target e event.currentTarget?"
    options:
      - "Não há diferença — são sempre o mesmo elemento"
      - "event.target é sempre o document; event.currentTarget é o elemento clicado"
      ...
      - "event.target existe apenas em eventos de teclado; event.currentTarget em eventos de mouse"
    correct: 2
  ...
  - question: "Por que delegação de eventos é eficiente para listas dinâmicas?"
    options:
      - "Porque um listener no pai é mais rápido que N listeners nos filhos"
      ...
      - "Porque event.target não está disponível em listeners individuais"
      - "Porque delegação funciona apenas com eventos de clique"
    correct: 1
  - question: "O que window.addEventListener('scroll', handler) permite fazer?"
    options:
      ...
      - "Detectar o scroll do mouse sobre qualquer elemento"
      - "Criar um scroll suave automaticamente"
      - "Bloquear o scroll do usuário"
    correct: 0
---
