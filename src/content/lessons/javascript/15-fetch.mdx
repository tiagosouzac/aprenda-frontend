...
quiz:
  - question: "Por que fetch não rejeita automaticamente para respostas 4xx e 5xx?"
    options:
      - "É um bug do fetch que será corrigido"
      - "Porque 4xx e 5xx não são erros reais do HTTP"
      ...
      - "Porque fetch só funciona com APIs que retornam 2xx"
    correct: 2
  - question: "Por que há dois awaits em: const dados = await (await fetch(url)).json()?"
    options:
      - "O segundo await é um erro — await só pode ser usado uma vez por linha"
      - "O primeiro await baixa o arquivo; o segundo o descomprime"
      - "São equivalentes — qualquer um dos dois é suficiente"
  ...
  - question: "O que o header Content-Type: application/json faz em uma requisição POST?"
    options:
      ...
      - "Converte o body para JSON automaticamente"
      - "É obrigatório apenas com PUT, não com POST"
    correct: 1
  - question: "O que é response.ok?"
    options:
      - "true se o status HTTP está entre 200 e 299 — indica sucesso"
      - "true se a resposta foi recebida em menos de 200ms"
      - "true se o body da resposta não está vazio"
      - "true se a requisição usou HTTPS"
    correct: 0
---
