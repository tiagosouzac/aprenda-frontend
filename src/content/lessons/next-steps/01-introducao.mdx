...
quiz:
  - question: "O que o conhecimento de HTML, CSS e JavaScript juntos permite fazer que cada um separado não permite?"
    options:
      - "Permite criar e administrar bancos de dados relacionais completos, com tabelas, relacionamentos e consultas SQL bem otimizadas"
      - "Permite publicar aplicativos nativos diretamente nas lojas de aplicativos do Android e do iOS, já prontos para download pelo usuário"
      - "Construir qualquer interface web do zero — estrutura (HTML), aparência (CSS) e comportamento (JS) são as três camadas de qualquer site. Juntos, você pode reproduzir a UI de qualquer produto que abre no browser"
      - "Permite construir APIs REST completas no lado do servidor, com rotas, autenticação e acesso a um banco de dados por trás de tudo"
    correct: 2
  - question: "Por que TypeScript foi introduzido depois de JavaScript no curso, e não antes?"
    options:
      - "Porque o TypeScript é uma linguagem bem mais difícil de aprender e exige bastante experiência prévia com programação para ser usado"
      - "Porque o TypeScript e o JavaScript são, na verdade, duas linguagens totalmente independentes uma da outra e sem nenhuma relação direta"
      - "Porque o TypeScript só passa a ser realmente útil quando você está trabalhando especificamente com a biblioteca React em um projeto"
      - "TypeScript é uma camada sobre JavaScript — para entender o que TypeScript adiciona (tipos, interfaces, generics), é preciso conhecer o JavaScript que ele está tipando. A ordem mostra o problema antes da solução"
    correct: 3
  - question: "O que o curso não cobriu e que faz parte do desenvolvimento web real?"
    options:
      - "O curso deixou de cobrir apenas coisas bem simples, como a forma correta de usar o navegador no dia a dia do desenvolvimento web"
      - "Roteamento entre páginas em React, renderização no servidor (SSR/SSG), comunicação com banco de dados, deploy automatizado, autenticação e autorização"
      - "O curso deixou de cobrir apenas o básico, como escrever HTML, CSS e JavaScript, que na verdade acabaram sendo o foco principal de tudo"
      - "O curso deixou de cobrir apenas detalhes menores, como a forma correta de abrir e de usar o devtools do navegador no dia a dia"
    correct: 1
---
