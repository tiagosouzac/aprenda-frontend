...
quiz:
  - question: "Qual é a principal vantagem do Tailwind CSS em relação ao CSS puro?"
    options:
      - "Utility-first: classes atômicas, sem nomear seletores; o bundle final só traz as classes usadas"
      - "O Tailwind gera um CSS final mais rápido de executar no navegador"
      - "O Tailwind permite estilizar tudo sem nenhum arquivo CSS no projeto"
      - "O Tailwind oferece um suporte a animações bem melhor que o CSS puro"
    correct: 0
  - question: "O que CSS Modules resolve que CSS global não resolve?"
    options:
      - "Os CSS Modules dão suporte a variáveis, algo que o CSS global não tem"
      - "Os CSS Modules são mais rápidos de executar que o CSS global comum"
      - "Gera nomes de classe únicos em build time, então os estilos de um componente não vazam"
      - "Os CSS Modules funcionam sem nenhuma configuração extra no projeto"
    correct: 2
  - question: "Qual é o problema de styled-components em comparação com soluções build-time?"
    options:
      - "O styled-components não oferece nenhum suporte ao uso de TypeScript"
      - "O styled-components não dá suporte a temas claro e escuro na página"
      - "A sintaxe do styled-components é mais verbosa que as outras soluções"
      - "Injeta estilos em runtime via JS: aumenta o bundle, pode causar FOUC e complica o SSR"
    correct: 3
  - question: "Quando faz sentido usar CSS global em vez de qualquer solução de escopo?"
    options:
      - "Nunca: o CSS global sempre causa algum conflito de estilos no projeto"
      - "Para tokens de design, reset/normalize, fontes e estilos de elementos base"
      - "Apenas nos projetos pequenos o suficiente em quantidade de código"
      - "Apenas quando o projeto usa um framework como o Next.js por baixo"
    correct: 1
---
