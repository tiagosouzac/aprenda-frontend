...
quiz:
  - question: "O que o backend faz que o frontend não pode fazer?"
    options:
      ...
      - "O backend é o único que consegue exibir o HTML já renderizado dentro do navegador do usuário que está acessando aquela página"
      - "O backend é o único capaz de executar código JavaScript, já que o frontend simplesmente não tem acesso a nenhum motor de execução"
      - "O backend é o único que consegue de fato fazer requisições HTTP para outros servidores e serviços que estão espalhados pela internet"
    correct: 0
  - question: "Qual é a diferença entre REST e GraphQL?"
    options:
      - "O REST sempre troca dados usando o formato JSON, enquanto o GraphQL troca todos os seus dados usando o formato XML por padrão"
      - "O REST é, por sua própria natureza, uma abordagem bem mais segura de se construir uma API do que o GraphQL em qualquer projeto"
      ...
      - "O GraphQL é, em todos os casos possíveis, perceptivelmente mais rápido para responder a uma requisição do que uma API REST comum"
    correct: 2
  - question: "Para que serve um ORM como Prisma?"
    options:
      - "Um ORM como o Prisma serve principalmente para criar as interfaces de usuário de uma aplicação web a partir das tabelas do banco"
      - "Um ORM como o Prisma serve para gerar, de forma totalmente automática, uma API REST completa a partir das tabelas do banco de dados"
      - "Um ORM como o Prisma serve essencialmente para conectar o código do frontend diretamente ao código do backend daquela aplicação"
  ...
  - question: "O que é um BFF (Backend for Frontend)?"
    options:
      - "É uma biblioteca específica criada para construir o código de servidores e de backends inteiros usando a própria sintaxe do React"
      ...
      - "É um tipo de banco de dados que foi especialmente otimizado para ser usado em conjunto com aplicações que são construídas em React"
      - "É exatamente a mesma coisa que uma API REST convencional, apenas sendo chamada por um outro nome um pouco mais moderno no mercado"
    correct: 1
---
