...
quiz:
  - question: "O que o backend faz que o frontend não pode fazer?"
    options:
      - "Acessar o banco, guardar segredos e rodar lógica que não pode ficar exposta no cliente"
      - "Exibir o HTML já renderizado dentro do navegador do usuário"
      - "Executar código JavaScript, coisa que o frontend não consegue fazer"
      - "Fazer requisições HTTP para outros servidores e serviços da internet"
    correct: 0
  - question: "Qual é a diferença entre REST e GraphQL?"
    options:
      - "O REST troca dados em JSON e o GraphQL troca tudo em XML por padrão"
      - "O REST é, por natureza, uma forma bem mais segura de API que o GraphQL"
      - "REST tem um endpoint por recurso; GraphQL tem um só e o cliente pede os campos exatos"
      - "O GraphQL é sempre mais rápido para responder que uma API REST comum"
    correct: 2
  - question: "Para que serve um ORM como Prisma?"
    options:
      - "Criar as interfaces de usuário da aplicação a partir das tabelas do banco"
      - "Gerar automaticamente uma API REST completa a partir das tabelas do banco"
      - "Conectar diretamente o código do frontend ao código do backend da aplicação"
      - "Acessar o banco com JavaScript/TypeScript em vez de SQL puro, com tipos e migrações"
    correct: 3
  - question: "O que é um BFF (Backend for Frontend)?"
    options:
      - "Uma biblioteca para construir servidores e backends com a sintaxe do React"
      - "Uma camada de backend dedicada a um frontend: agrega várias APIs no formato exato que a tela precisa"
      - "Um tipo de banco de dados otimizado para aplicações feitas em React"
      - "É exatamente a mesma coisa que uma API REST convencional, só com outro nome"
    correct: 1
---
