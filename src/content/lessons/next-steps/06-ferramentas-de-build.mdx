...
quiz:
  - question: "O que uma ferramenta de build faz com o código TypeScript e JSX antes de entregar ao browser?"
    options:
      ...
      - "Ela apenas executa o código uma única vez e então salva o resultado final dessa execução em um cache para reaproveitar depois"
      - "Ela apenas faz a verificação dos tipos do código usando o TypeScript, sem realizar nenhuma outra transformação no código-fonte"
      - "Ela apenas comprime e reduz o tamanho dos arquivos do projeto, sem chegar a modificar de fato o conteúdo do código que foi escrito"
    correct: 0
  - question: "Por que o Vite é mais rápido no desenvolvimento que ferramentas baseadas em bundle??"
    options:
      - "Porque o Vite utiliza, por baixo dos panos, um compilador interno bem mais eficiente do que o usado pelas outras ferramentas de build"
      - "Porque o Vite escreve todo o bundle gerado diretamente na memória RAM da máquina, em vez de salvá-lo em arquivos no disco rígido"
      ...
      - "Porque o Vite consegue distribuir o trabalho de build entre várias threads diferentes da CPU ao mesmo tempo durante a sua execução"
    correct: 2
  - question: "O que é tree-shaking?"
    options:
      - "É uma técnica usada especificamente para otimizar e reduzir o tamanho das imagens que são carregadas pela aplicação dentro do navegador"
      - "É uma técnica usada para dividir o bundle final da aplicação em vários pedaços (chunks) menores, que são carregados sob demanda"
      - "É o processo responsável por minificar todo o CSS do projeto, removendo dele os espaços e os comentários antes de ele ir para produção"
  ...
  - question: "Quando você precisa se aprofundar em configuração de build?"
    options:
      - "Você precisa se aprofundar em configuração de build sempre, porque absolutamente todo projeto acaba precisando de uma configuração personalizada"
      ...
      - "Você só precisa se aprofundar em configuração de build quando o projeto em questão chega a ter mais de cem arquivos diferentes no total"
      - "Você só precisa se aprofundar em configuração de build especificamente quando o projeto está usando o webpack como a sua ferramenta de build"
    correct: 1
---
