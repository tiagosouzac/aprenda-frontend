...
quiz:
  - question: "O que é CI (Continuous Integration) e qual o seu benefício principal?"
    options:
      ...
      - "É um serviço de hospedagem de código-fonte na nuvem, bem parecido com o que o próprio GitHub já oferece para os repositórios"
      - "É um sistema de controle de versão de código, usado para acompanhar o histórico de todas as mudanças que são feitas no projeto"
      - "É, de forma resumida, apenas a automação de todo o processo de fazer o deploy de uma aplicação para o ambiente de produção"
    correct: 0
  - question: "Por que o blog React (SPA) precisa de configuração especial de rotas no Vercel/Netlify?"
    options:
      - "Porque a biblioteca React simplesmente não consegue funcionar de jeito nenhum sem ter um servidor ativo rodando por trás dela"
      - "Porque a ferramenta de build Vite acaba gerando um arquivo HTML separado e diferente para cada uma das rotas da aplicação"
      ...
      - "Porque o serviço de hospedagem Netlify, por algum motivo de compatibilidade, não oferece nenhum tipo de suporte a aplicações em React"
    correct: 2
  - question: "O que são preview deployments e por que são úteis?"
    options:
      - "São versões do ambiente de produção que foram otimizadas para carregar de forma perceptivelmente mais rápida para o usuário final"
      - "São versões especiais do site que rodam com dados totalmente fictícios, criadas apenas para a realização de testes manuais da equipe"
      - "São deploys que acabam rodando exclusivamente dentro de um ambiente de staging, separado por completo do ambiente de produção real"
  ...
  - question: "Por que variáveis de ambiente com segredos não devem ser commitadas no repositório?"
    options:
      - "Porque o próprio Git, por uma limitação técnica, simplesmente não oferece nenhum suporte ao versionamento de arquivos com a extensão .env"
      ...
      - "Porque as variáveis de ambiente, por uma questão técnica, acabam não funcionando de forma correta quando a aplicação já está rodando em produção"
      - "É apenas por uma questão de convenção entre os desenvolvedores: na prática, não existe nenhum risco técnico real em commitar os segredos no repositório"
    correct: 1
---
