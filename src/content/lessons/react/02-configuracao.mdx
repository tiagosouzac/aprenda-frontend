...
quiz:
  - question: "Por que Vite é preferível ao Create React App para projetos novos?"
    options:
      - "Vite usa esbuild: o dev server inicia em ms e o HMR é quase instantâneo; o CRA usa webpack e fica lento"
      - "Porque o Vite é mantido pela própria equipe do React, garantindo integração perfeita"
      - "Porque o Create React App foi descontinuado e removido do npm, deixando de funcionar"
      - "Porque o Vite dispensa o tsconfig.json e configura o TypeScript sozinho"
    correct: 0
  - question: "O que o arquivo main.tsx faz?"
    options:
      - "É o arquivo que define todos os estilos globais que valem para a aplicação inteira"
      - "É onde ficam declaradas as rotas e o mapeamento de cada URL para uma tela"
      - "É o ponto de entrada — monta o App no #root do index.html com createRoot().render()"
      - "É um índice que reexporta todos os componentes para encurtar os imports"
    correct: 2
  - question: "Por que artigos.json vai na pasta public/ em vez de src/?"
    options:
      - "Porque arquivos JSON não podem ser importados em TypeScript e ficam fora da pasta src"
      - "Porque a pasta public é servida de um disco mais rápido que a src, acelerando o carregamento"
      - "Porque a pasta src só aceita .ts e .tsx, e ignora qualquer .json colocado dentro dela"
      - "Arquivos em public/ são servidos via URL (fetch('/artigos.json')), não via import como os de src/"
    correct: 3
  - question: "O que o operador ! (non-null assertion) faz em document.getElementById('root')!?"
    options:
      - "Cria automaticamente o elemento #root na página caso ele ainda não exista no HTML"
      - "Diz ao TS que você garante que o valor não é null; se faltar em runtime, o erro aparece aí"
      - "Lança um erro em tempo de compilação quando o elemento não puder ser encontrado"
      - "Converte o elemento para o tipo HTMLElement automaticamente, sem anotação manual"
    correct: 1
---
