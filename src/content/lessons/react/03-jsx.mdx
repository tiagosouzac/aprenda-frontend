...
quiz:
  - question: "O que é JSX na prática?"
    options:
      - "Açúcar sintático sobre React.createElement() — o browser nunca vê JSX diretamente"
      - "Uma versão aprimorada do HTML, criada pelo Facebook, que os navegadores já interpretam"
      - "Um arquivo separado, com extensão própria, que descreve apenas a estrutura visual"
      - "Uma linguagem de template que roda no servidor e devolve o HTML já montado"
    correct: 0
  - question: "Por que class vira className em JSX?"
    options:
      - "Por uma convenção de estilo do React, para diferenciar dos atributos do HTML"
      - "Porque className é processado mais rápido pelo navegador que o class tradicional"
      - "class é palavra reservada do JS, e o JSX compila para JS, que exige identificadores válidos"
      - "Porque class só funciona em elementos HTML nativos e quebraria em um componente"
    correct: 2
  - question: "Qual é o problema de usar {0 && <Componente />} para renderização condicional?"
    options:
      - "Não existe problema: a expressão não renderiza nada quando a condição é falsa"
      - "O componente é renderizado mesmo quando a condição à esquerda do && é falsy"
      - "O JSX não dá suporte ao operador && em expressões, gerando erro de sintaxe"
      - "Quando a condição é 0, o React renderiza o 0 no DOM; use Boolean(0) ou 0 > 0 para evitar"
    correct: 3
  - question: "Quando usar um fragmento (<>...</>) em vez de uma <div> como wrapper?"
    options:
      - "Sempre que possível, porque fragmentos são renderizados mais rápido que qualquer elemento"
      - "Quando você retorna vários elementos sem adicionar um nó extra ao DOM que pode quebrar o CSS"
      - "Apenas ao renderizar listas, situação em que a div não é aceita como elemento pai"
      - "Só em componentes de página, já que fragmentos não funcionam em componentes menores"
    correct: 1
---
