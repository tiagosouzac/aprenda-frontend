...
quiz:
  - question: "Por que componentes React têm nome em PascalCase?"
    options:
      - "O compilador JSX usa a capitalização para distinguir um componente de uma tag HTML"
      - "É só uma convenção de estilo, sem nenhum efeito sobre como o componente é compilado"
      - "Porque nomes em PascalCase são processados mais rápido pelo React na renderização"
      - "Por exigência do TypeScript, que recusa componentes cujo nome comece com minúscula"
    correct: 0
  - question: "Por que named exports são preferíveis a default exports para componentes?"
    options:
      - "Porque default exports não funcionam em projetos TypeScript e geram erro de compilação"
      - "Porque named exports são carregados mais rápido pelo bundler e reduzem o pacote final"
      - "Obrigam o importador a usar o nome certo, então o Rename Symbol funciona; default aceita qualquer nome"
      - "Porque o Vite não dá suporte a default exports e exige sempre um nome explícito"
    correct: 2
  - question: "O que significa retornar null de um componente React?"
    options:
      - "Provoca um erro em runtime, já que todo componente é obrigado a retornar algum JSX válido"
      - "Faz o componente ser removido em definitivo da árvore e precisar ser remontado do zero"
      - "Renderiza um <span> vazio no lugar, presente no DOM mas sem nenhum conteúdo visível"
      - "O componente não renderiza nada no DOM — uma forma válida de escondê-lo sem removê-lo da árvore"
    correct: 3
  - question: "Qual é a vantagem de compor componentes pequenos em vez de um componente grande?"
    options:
      - "Componentes menores são sempre renderizados mais rápido, pois o React processa funções curtas antes"
      - "Cada um tem responsabilidade clara, é testável e reutilizável, e mais fácil de entender sem ler tudo"
      - "Porque o React impõe um limite de cem linhas por componente, e acima disso não compila"
      - "Componentes menores consomem bem menos memória em runtime, melhorando o desempenho"
    correct: 1
---
