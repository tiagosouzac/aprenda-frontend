...
quiz:
  - question: "Em que direção os dados fluem via props em React?"
    options:
      - "Apenas de pai para filho (unidirecional); para comunicar de volta, use callbacks via props"
      - "Em qualquer direção: as props vão de pai para filho e de filho de volta para o pai"
      - "Apenas do componente raiz direto para as folhas, pulando os intermediários da árvore"
      - "De forma bidirecional sempre que o componente que recebe a prop usar useState"
    correct: 0
  - question: "Por que props são imutáveis dentro do componente que as recebe?"
    options:
      - "Por uma limitação do JavaScript, que bloqueia a reatribuição de qualquer parâmetro"
      - "Porque o TypeScript marca todas as props como readonly e impede a mutação na compilação"
      - "Mutar a prop tornaria o fluxo imprevisível: o pai não saberia da mudança; derive com useState"
      - "É só uma convenção recomendada pela documentação, sem efeito prático sobre o componente"
    correct: 2
  - question: "O que é children em React?"
    options:
      - "Um array com todos os componentes filhos da aplicação naquele instante do render"
      - "Um hook especial para acessar os elementos do DOM renderizados pelo componente filho"
      - "Uma referência ao componente pai, para o filho ler as props de quem o renderizou"
      - "A prop especial que recebe o conteúdo entre as tags do componente — <Card>conteúdo aqui</Card>"
    correct: 3
  - question: "Qual é a diferença entre desestruturar props no parâmetro vs. acessar via objeto props?"
    options:
      - "Desestruturar direto no parâmetro é sempre mais rápido em runtime do que acessar pelo objeto props"
      - "São equivalentes; desestruturar é mais conciso, e props.titulo ajuda a repassar tudo com spread"
      - "Você só consegue usar spread com o objeto props inteiro, nunca com props desestruturadas"
      - "O TypeScript só dá suporte à forma desestruturada e acusa erro ao acessar via props.titulo"
    correct: 1
---
