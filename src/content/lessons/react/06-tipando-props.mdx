...
quiz:
  - question: "Por que extends React.ButtonHTMLAttributes<HTMLButtonElement> é útil?"
    options:
      - "Para o componente aceitar todos os atributos nativos do botão sem redeclará-los um a um"
      - "Para o componente herdar automaticamente o comportamento visual e os cliques do botão"
      - "Apenas para manter compatibilidade com versões antigas do React que não tipavam nativos"
      - "Apenas para garantir que o evento onClick do botão funcione dentro do componente"
    correct: 0
  - question: "Qual é a diferença entre ReactNode e ReactElement?"
    options:
      - "São sinônimos perfeitos: dá para usar qualquer um dos dois sem nenhuma diferença prática"
      - "ReactElement aceita null entre os filhos, enquanto ReactNode recusa null e undefined"
      - "ReactNode é mais amplo (JSX, string, número, array, null); ReactElement é só o resultado de JSX"
      - "ReactNode é só para componentes de classe e ReactElement só para os de função"
    correct: 2
  - question: "Qual é o tipo correto para o handler do evento onChange de um input?"
    options:
      - "O tipo Event genérico do DOM, o mesmo do addEventListener, sem tipagem específica do React"
      - "O tipo HTMLInputElement, que descreve o input em si e não o evento de mudança do valor"
      - "O tipo InputEvent nativo do navegador, que representa o texto digitado dentro do campo"
      - "React.ChangeEvent<HTMLInputElement> — o evento sintético do React tipado para o elemento"
    correct: 3
  - question: "O que React.ComponentProps<'button'> retorna?"
    options:
      - "O tipo do componente button em si, a assinatura da função que o React usa para renderizá-lo"
      - "O tipo de todas as props que o <button> nativo aceita — versão curta de ButtonHTMLAttributes"
      - "O tipo do valor devolvido pela função button quando ela é executada na renderização"
      - "Um objeto com todos os métodos que o button expõe para serem chamados via referência"
    correct: 1
---
