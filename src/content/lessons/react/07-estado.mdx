...
quiz:
  - question: "Por que uma variável comum não funciona para atualizar a UI em React?"
    options:
      - "Mudar uma variável comum não avisa o React, então o componente não re-renderiza"
      - "Variáveis dentro do componente são somente leitura, e o React lança erro ao reatribuir"
      - "O React executa o corpo do componente uma vez, e a variável local nunca é recalculada"
      - "Variáveis comuns vivem fora da árvore, e o React só lê valores que estão nas props"
    correct: 0
  - question: "Por que usar setContagem(prev => prev + 1) em vez de setContagem(contagem + 1)?"
    options:
      - "A forma funcional é compilada de modo mais eficiente e reduz as re-renderizações por clique"
      - "A forma direta quebra a inferência do TypeScript e obriga a anotar o tipo do estado"
      - "O React agrupa várias atualizações no mesmo ciclo, e a forma funcional sempre parte do valor mais recente"
      - "Não há diferença prática: a forma funcional é só uma convenção de estilo da documentação"
    correct: 2
  - question: "O que significa a regra 'nunca chame hooks dentro de condicionais'?"
    options:
      - "Que hooks não podem ser usados em nenhum componente que contenha um if/else no corpo"
      - "Que um hook não consegue ler variáveis declaradas dentro de um if anterior no mesmo render"
      - "É só uma recomendação de estilo para o componente ficar mais legível, sem impacto real"
      - "A ordem das chamadas de hook deve ser a mesma em todo render; um if pode pular um hook e corromper isso"
    correct: 3
  - question: "Ao atualizar um objeto no estado, por que você deve criar um novo objeto em vez de mutar o existente?"
    options:
      - "Porque o TypeScript marca todo objeto em estado como readonly e impede a mutação na compilação"
      - "O React compara o estado por referência; mutar mantém a mesma referência e ele não detecta a mudança"
      - "Porque mutar um objeto que está no estado lança um erro de runtime que derruba a renderização"
      - "Porque é uma prática herdada do Redux, mas no React puro mutar o objeto funciona igual"
    correct: 1
---
