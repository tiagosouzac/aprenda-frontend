...
quiz:
  - question: "Qual é o risco de usar {lista.length && <Componente />} em vez de {lista.length > 0 && <Componente />}?"
    options:
      ...
      - "Nenhum — as duas formas são equivalentes"
      - "A primeira forma não compila com TypeScript"
      - "A segunda forma é mais lenta"
    correct: 0
  - question: "Por que usar o índice do array como key é problemático quando a lista pode ser reordenada?"
    options:
      - "O índice não é permitido pelo TypeScript"
      - "O índice muda entre renders e isso causa erros de runtime"
      ...
      - "Não é problemático — é apenas uma convenção"
    correct: 2
  - question: "Qual é a diferença entre {condição && <A />} e {condição ? <A /> : null}?"
    options:
      - "O operador && nunca renderiza null"
      - "O ternário não funciona com componentes"
      - "O && é mais rápido"
  ...
  - question: "Onde a prop key deve ser colocada?"
    options:
      - "Dentro do componente que a recebe, via props.key"
      - "No elemento raiz retornado pelo .map() — não é acessível como prop dentro do componente"
      - "Em qualquer elemento da lista, não precisa ser o raiz"
      - "No componente pai que renderiza a lista"
    correct: 1
---
