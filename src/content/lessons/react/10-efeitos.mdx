...
quiz:
  - question: "Qual é a diferença entre useEffect sem array de dependências e useEffect com array vazio []?"
    options:
      ...
      - "Nenhuma diferença entre as duas formas: tanto sem array quanto com array vazio o efeito roda uma única vez na montagem"
      - "O array vazio faz com que o efeito seja executado antes do render do componente, em vez de depois que o DOM é atualizado"
      - "Sem o array de dependências, o efeito perde o acesso ao estado e às props atuais do componente durante a sua execução"
    correct: 0
  - question: "Para que serve a função de cleanup retornada pelo useEffect?"
    options:
      - "Serve para reinicializar todo o estado do componente de volta ao valor inicial antes que o próximo render aconteça"
      - "Serve para limpar o cache interno que o React mantém das renderizações anteriores daquele componente específico"
      ...
      - "Serve para reverter o DOM ao exato estado em que ele estava antes de o efeito ter sido executado pela primeira vez"
    correct: 2
  - question: "Por que você não deve usar useEffect para derivar estado de props?"
    options:
      - "Porque o useEffect não tem acesso às props do componente, o que torna impossível derivar qualquer estado a partir delas"
      - "Porque o TypeScript não permite usar um useEffect que dependa de props e acusa um erro de compilação nessa situação"
      - "É permitido derivar estado dentro do useEffect, mas o resultado fica perceptivelmente mais lento do que calcular no render"
  ...
  - question: "O que acontece se você não declarar todas as dependências que o efeito usa no array de dependências?"
    options:
      - "O TypeScript avisa com um erro de compilação sempre que uma dependência usada pelo efeito fica de fora do array informado"
      ...
      - "O React lança um erro em tempo de execução assim que o efeito tenta ler um valor que não foi declarado nas dependências"
      - "Não há consequência alguma: o React detecta sozinho as dependências usadas e mantém o efeito sempre com os valores atuais"
    correct: 1
---
