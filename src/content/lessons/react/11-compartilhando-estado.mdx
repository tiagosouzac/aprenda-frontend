...
quiz:
  - question: "O que significa 'lifting state up' em React?"
    options:
      ...
      - "É a técnica de usar um gerenciador de estado global como o Redux para que qualquer componente acesse o valor de qualquer lugar"
      - "É guardar o estado no localStorage do navegador para que os componentes consigam compartilhar o mesmo valor entre si"
      - "É duplicar o mesmo estado em cada componente que precisa dele e manter todas as cópias sincronizadas manualmente a cada mudança"
    correct: 0
  - question: "O que é prop drilling e quando começa a ser problemático?"
    options:
      - "É quando você passa props diretamente de um pai para o seu filho imediato — e, segundo a regra, isso é sempre problemático"
      - "É um problema que só acontece quando o valor repassado entre os componentes da árvore é especificamente um array de itens"
      ...
      - "É um padrão recomendado pela documentação do React para compartilhar dados em qualquer profundidade da árvore de componentes"
    correct: 2
  - question: "Como um componente filho comunica eventos ao seu pai em React?"
    options:
      - "Mutando diretamente a prop que recebeu, de modo que a alteração suba de volta para o componente pai que a enviou"
      - "Usando uma variável global declarada fora dos componentes, que o filho escreve e o pai lê quando precisa reagir à mudança"
      - "Chamando um método exposto pelo componente pai por meio de uma ref, da mesma forma que se faz com um elemento do DOM"
  ...
  - question: "Quais são os sinais de que o estado está no lugar errado na árvore?"
    options:
      - "Quando o componente passa a declarar mais de um useState ao mesmo tempo dentro do corpo da sua função"
      ...
      - "Quando o valor guardado no estado é um objeto ou um array, em vez de um valor primitivo como um número ou uma string"
      - "Quando o componente que detém o estado renderiza uma quantidade grande de componentes filhos dentro de si"
    correct: 1
---
