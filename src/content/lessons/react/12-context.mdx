...
quiz:
  - question: "O que é o Context no React e para que serve?"
    options:
      ...
      - "É um banco de dados interno que o React mantém para persistir os dados da aplicação entre diferentes sessões do usuário"
      - "É um substituto para o useEffect, usado especificamente quando você precisa sincronizar dados vindos de uma fonte externa"
      - "É uma forma de comunicação direta entre componentes irmãos, permitindo que um envie dados ao outro sem passar pelo pai"
    correct: 0
  - question: "O que acontece quando o value de um Provider muda?"
    options:
      - "Apenas o próprio componente Provider é re-renderizado, sem afetar nenhum dos componentes que consomem aquele contexto"
      - "Apenas os componentes que são filhos diretos do Provider re-renderizam, e os que estão mais profundos na árvore permanecem intactos"
      - "Todos os componentes que consomem aquele Context com useContext re-renderizam, independentemente de onde estão na árvore"
      - "Nada acontece na tela, porque o valor exposto por um Context é imutável depois que o Provider é montado pela primeira vez"
    correct: 2
  - question: "Por que é boa prática criar um custom hook para encapsular useContext?"
    options:
      - "Para que o TypeScript consiga inferir corretamente os tipos do valor que o contexto retorna dentro de cada componente"
      - "Porque o useContext não pode ser chamado diretamente em um componente e precisa sempre estar embrulhado dentro de outro hook"
      - "Para evitar que os componentes que consomem o valor exposto por aquele contexto sofram re-renderizações desnecessárias"
  ...
  - question: "Quando Context NÃO é a solução certa?"
    options:
      - "Sempre que você tiver mais de dois componentes diferentes precisando ler e usar exatamente o mesmo valor compartilhado"
      ...
      - "Quando o valor que você precisa compartilhar entre os componentes é um objeto, em vez de um valor primitivo mais simples"
      - "Nunca: o Context é sempre a solução certa para qualquer caso de estado que precise ser compartilhado entre componentes"
    correct: 1
---
