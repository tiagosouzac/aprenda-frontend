...
quiz:
  - question: "Por que todo fetch em React precisa dos três estados: carregando, erro e dados?"
    options:
      ...
      - "É uma exigência técnica do useEffect"
      - "Para compatibilidade com TypeScript"
      - "Apenas carregando e dados são necessários — erro é opcional"
    correct: 0
  - question: "O que é uma race condition em fetch e como AbortController resolve?"
    options:
      - "Um bug que acontece quando dois usuários fazem requests ao mesmo tempo"
      - "AbortController não resolve race conditions — apenas cancela fetches em andamento"
      ...
      - "Race conditions acontecem apenas com useState, não com fetch"
    correct: 2
  - question: "Qual é a vantagem de extrair a lógica de fetch para um custom hook?"
    options:
      - "Custom hooks são mais rápidos que usar useEffect diretamente"
      - "Custom hooks evitam re-renderizações"
      - "É a única forma de usar fetch com TypeScript"
  ...
  - question: "O que TanStack Query oferece que useEffect + fetch manual não oferece?"
    options:
      - "Suporte a TypeScript — useEffect não suporta"
