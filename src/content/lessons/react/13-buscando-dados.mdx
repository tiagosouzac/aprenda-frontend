...
quiz:
  - question: "Por que todo fetch em React precisa dos três estados: carregando, erro e dados?"
    options:
      - "Uma requisição tem três desfechos: carregando, erro ou sucesso; sem eles a UI não sabe o que mostrar"
      - "É uma exigência técnica do useEffect, que não roda sem três estados separados declarados"
      - "É necessário só por compatibilidade com o TypeScript, que exige os três para tipar a resposta"
      - "Na prática só carregando e dados importam: o estado de erro é opcional e pode ser ignorado"
    correct: 0
  - question: "O que é uma race condition em fetch e como AbortController resolve?"
    options:
      - "É um bug que só acontece quando dois usuários fazem requisições ao mesmo endpoint ao mesmo tempo"
      - "O AbortController não resolve de fato: ele só cancela fetches, sem influenciar qual resultado vence"
      - "Um fetch antigo pode terminar depois de um novo e sobrescrever o resultado; o AbortController cancela o anterior"
      - "Race conditions só acontecem quando você usa useState, e nunca em código que faz fetch"
    correct: 2
  - question: "Qual é a vantagem de extrair a lógica de fetch para um custom hook?"
    options:
      - "Os custom hooks são executados mais rápido pelo React do que usar o useEffect direto"
      - "Os custom hooks evitam que os componentes que usam o fetch sofram re-renderizações à toa"
      - "É a única forma de usar fetch com TypeScript, já que o useEffect direto não aceita tipos"
      - "Reutilizar a lógica de carregando/erro/dados em vários componentes sem duplicar código"
    correct: 3
  - question: "O que TanStack Query oferece que useEffect + fetch manual não oferece?"
    options:
      - "Oferece suporte a TypeScript, algo que o useEffect com fetch manual não dá ao seu código"
      - "Cache automático, deduplicação, revalidação em background e estados já gerenciados — menos boilerplate"
      - "Oferece suporte a requisições POST, que não dá para fazer com fetch dentro de um useEffect"
      - "Oferece cancelamento automático de requisições, um recurso impossível com fetch manual"
    correct: 1
---
