...
quiz:
  - question: "Por que o filtro de artigos por query e tag não usa useEffect?"
    options:
      - "O filtro é uma derivação dos dados já disponíveis, calculável no render; useEffect seria um render a mais"
      - "Porque o useEffect é lento demais para filtros e travaria a interface a cada caractere digitado"
      - "Porque arrays de objetos não podem ser manipulados dentro de um useEffect, só primitivos"
      - "Na verdade o useEffect é necessário para qualquer filtro, então o exemplo está incorreto"
    correct: 0
  - question: "Por que o estado de curtidas vive no Context e não dentro de cada ArticleCard?"
    options:
      - "Por uma limitação do React, que não deixa guardar esse estado em componentes de lista"
      - "Porque o Context é sempre mais performático que o useState local e reduz as renderizações"
      - "As curtidas são compartilhadas entre HomePage e ArticlePage; estado local ficaria dessincronizado"
      - "Porque o ArticleCard, por ser renderizado em um .map(), não pode chamar o hook useState"
    correct: 2
  - question: "O que é necessário para que a barra de progresso de leitura funcione com scroll?"
    options:
      - "Um timer com setInterval que checa a posição de scroll em intervalos e atualiza a porcentagem"
      - "Uma biblioteca externa de animação que observa a rolagem e preenche a barra de progresso"
      - "A propriedade scroll-timeline do CSS, que resolve o progresso de leitura sem JavaScript"
      - "Um useEffect com listener de 'scroll' que calcula a porcentagem lida e limpa o listener no cleanup"
    correct: 3
  - question: "Qual é o motivo de separar a lógica de fetchArtigo em um custom hook useArtigo?"
    options:
      - "Porque o useEffect não pode ser chamado dentro de componentes que são páginas inteiras"
      - "Separar o que o componente renderiza do como os dados chegam — reutilizável e mais fácil de testar"
      - "Porque os custom hooks são obrigatórios sempre que uma página busca dados com fetch"
      - "Para o TypeScript conseguir inferir o tipo dos dados que chegam da API na renderização"
    correct: 1
---
