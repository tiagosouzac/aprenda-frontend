...
quiz:
  - question: "Por que o filtro de artigos por query e tag não usa useEffect?"
    options:
      ...
      - "Porque o useEffect é uma ferramenta lenta demais para aplicar filtros e acabaria travando a interface a cada caractere digitado"
      - "Porque arrays de objetos não podem ser manipulados dentro de um useEffect, que só aceita valores primitivos como dependências"
      - "Na verdade o useEffect é necessário para qualquer filtro de lista, então o exemplo desta lição está implementado de forma incorreta"
    correct: 0
  - question: "Por que o estado de curtidas vive no Context e não dentro de cada ArticleCard?"
    options:
      - "Por uma limitação técnica do React, que não deixa guardar o estado de curtidas dentro de componentes renderizados em lista"
      - "Porque o Context é sempre mais performático que o useState local e reduz o número de renderizações de cada ArticleCard na tela"
      ...
      - "Porque o componente ArticleCard, por ser renderizado dentro de um .map(), não tem permissão para chamar o hook useState"
    correct: 2
  - question: "O que é necessário para que a barra de progresso de leitura funcione com scroll?"
    options:
      - "Um timer criado com setInterval que verifica a posição de scroll da página em intervalos regulares e atualiza a porcentagem lida"
      - "Uma biblioteca externa de animação, responsável por observar a rolagem e preencher a barra de progresso conforme o usuário desce"
      - "A propriedade scroll-timeline do CSS, que resolve o cálculo do progresso de leitura inteiramente sem precisar de JavaScript"
  ...
  - question: "Qual é o motivo de separar a lógica de fetchArtigo em um custom hook useArtigo?"
    options:
      - "Porque o hook useEffect não pode ser chamado diretamente dentro de componentes que representam páginas inteiras do blog"
      ...
      - "Porque os custom hooks são obrigatórios sempre que uma página precisa buscar dados com fetch, segundo as regras dos hooks"
      - "Para que o TypeScript consiga inferir corretamente o tipo dos dados que chegam da API durante a renderização da página"
    correct: 1
---
