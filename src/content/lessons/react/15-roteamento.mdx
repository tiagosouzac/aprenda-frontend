...
description: "Por que SPAs precisam de um router, como configurar o React Router DOM e conectar as páginas do blog com Link, useParams e useNavigate."
order: 15
hero:
...
quiz:
  - question: "Por que usar <a href='/artigos'> comum em uma SPA React causa problema?"
    options:
      - "Porque React não suporta o elemento <a>"
      - "Porque o href precisa ser absoluto em SPAs"
      ...
      - "Porque o React Router bloqueia navegações com <a>"
    correct: 2
  - question: "Qual é a diferença entre Link e NavLink do React Router?"
    options:
      - "Link é para links externos; NavLink é para links internos"
      - "NavLink recarrega a página; Link não"
      ...
      - "Link é o componente antigo; NavLink é a versão nova que substituiu Link"
    correct: 2
  - question: "Como um componente acessa o parâmetro :slug da rota /artigos/:slug?"
    options:
      - "Via props: function ArticlePage({ slug })"
      - "Via useContext(RouterContext)"
      - "Via useParams: const { slug } = useParams()"
      - "Via useLocation: location.pathname.split('/')[2]"
    correct: 2
---
...
```bash title="Instalar o React Router DOM."
npm install react-router-dom
```

A API recomendada no React Router v6 usa `createBrowserRouter` — uma função que recebe a lista de rotas e retorna um objeto de router. O `RouterProvider` monta esse router na raiz da aplicação:
...
- `npm install react-router-dom` + `createBrowserRouter` + `RouterProvider`: configuração moderna do React Router v6.
