...
quiz:
  - question: "Por que usar <a href='/artigos'> comum em uma SPA React causa problema?"
    options:
      - "Porque o React não dá suporte ao <a> nativo e exige um componente próprio para navegar"
      - "Porque dentro de uma SPA o href precisa apontar sempre para um endereço absoluto completo"
      - "O navegador faz uma nova requisição e recarrega; a app perde o estado e remonta tudo do zero"
      - "Porque o React Router intercepta e bloqueia qualquer navegação feita por um <a> comum"
    correct: 2
  - question: "Qual é a diferença entre Link e NavLink do React Router?"
    options:
      - "O Link serve só para links externos, e o NavLink só para a navegação interna da app"
      - "O NavLink recarrega a página inteira a cada clique, enquanto o Link troca de rota sem reload"
      - "Ambos geram <a> sem reload; o NavLink ainda aplica uma classe quando a rota está ativa"
      - "O Link é o componente antigo do React Router e o NavLink veio para substituí-lo por completo"
    correct: 2
  - question: "Como um componente acessa o parâmetro :slug da rota /artigos/:slug?"
    options:
      - "Recebendo o slug como uma prop comum na assinatura: function ArticlePage({ slug })"
      - "Lendo o valor de um contexto interno do router, com useContext(RouterContext)"
      - "Via useParams, o hook do React Router para parâmetros de rota: const { slug } = useParams()"
      - "Extraindo manualmente da URL com useLocation: location.pathname.split('/')[2]"
    correct: 2
---
