...
quiz:
  - question: "Qual das afirmações sobre keys em listas React está correta?"
    options:
      - "Keys devem ser únicas na lista e estáveis; use o id do dado, não o índice, que causa bugs ao reordenar"
      - "As keys são totalmente opcionais, e o React funciona normalmente mesmo sem nenhuma nos itens"
      - "As keys precisam ser únicas em toda a aplicação, não só dentro do contexto daquela lista"
      - "As keys são entregues ao componente filho como uma prop comum, acessível por props.key"
    correct: 0
  - question: "Qual é a principal diferença entre usar useState local e Context?"
    options:
      - "O Context é sempre mais performático que o useState local e deveria ser o padrão para tudo"
      - "O Context não pode ser usado com TypeScript, porque não dá para tipar o valor que ele distribui"
      - "useState é para estado de um componente; Context compartilha entre vários níveis sem prop drilling"
      - "O useState local não funciona com efeitos, então estado lido num useEffect precisa vir do Context"
    correct: 2
  - question: "Por que os testes serão o próximo passo no curso?"
    options:
      - "Porque o React não funciona de forma confiável em produção sem uma suíte de testes acompanhando"
      - "Porque o próximo módulo do curso é, na verdade, sobre TypeScript, antes de avançar para os testes"
      - "Os testes são opcionais no aprendizado, e o próximo passo depois do React seria partir para o Next.js"
      - "Até aqui o blog só foi testado à mão; testes automatizados pegam regressões e documentam o esperado"
    correct: 3
---
