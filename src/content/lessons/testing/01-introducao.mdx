...
quiz:
  - question: "Qual é o principal benefício prático de ter testes automatizados?"
    options:
      - "Provar de forma matemática e definitiva que o código escrito não contém nenhum tipo de bug em nenhuma situação possível"
      ...
      - "Eliminar por completo a necessidade de revisão de código entre os pares da equipe durante todo o processo de desenvolvimento"
      - "Garantir que o projeto atinja cem por cento de cobertura de absolutamente todas as linhas de código existentes na base do projeto"
    correct: 1
  - question: "O que é uma regressão no contexto de software?"
    options:
      ...
      - "É uma técnica estatística de análise de dados bastante usada na área de machine learning para fazer previsões a partir de números"
      - "É um tipo específico de bug que só consegue se manifestar quando a aplicação já está rodando no ambiente de produção real"
      - "É um teste especial cuja função é verificar qual será o comportamento futuro que o sistema deverá ter nas próximas versões"
    correct: 0
  - question: "Por que testes não garantem ausência de bugs?"
    options:
      - "Porque os testes são escritos por seres humanos, e qualquer ser humano acaba cometendo erros de digitação ao escrevê-los no editor"
      - "Porque as ferramentas de teste disponíveis hoje ainda não são suficientemente precisas para conseguir capturar todos os bugs do código"
      ...
      - "Porque os testes verificam apenas a interface do usuário da aplicação, e nunca chegam a tocar na lógica de negócio por trás dela"
    correct: 2
  - question: "O que significa dizer que testes são 'documentação executável'?"
    options:
      - "Significa que os testes conseguem gerar arquivos de documentação em formato HTML de maneira totalmente automática a partir do código"
      - "Significa que você pode usar a sua suíte de testes no lugar de manter um arquivo README escrito à mão dentro do repositório do projeto"
      - "Significa que os testes servem para documentar qual é a porcentagem exata de cobertura de código que o projeto conseguiu atingir até agora"
