...
quiz:
  - question: "Qual é o principal benefício prático de ter testes automatizados?"
    options:
      - "Provar matematicamente que o código não contém nenhum bug"
      - "Confiança para mudar código — refatorar, corrigir e evoluir sem medo de quebrar algo em silêncio"
      - "Eliminar de vez a necessidade de revisão de código entre os pares da equipe"
      - "Garantir cem por cento de cobertura de todas as linhas do projeto"
    correct: 1
  - question: "O que é uma regressão no contexto de software?"
    options:
      - "Quando algo que funcionava para de funcionar após uma mudança não relacionada no código"
      - "Uma técnica estatística de análise de dados usada em machine learning"
      - "Um bug que só consegue se manifestar quando a aplicação está em produção"
      - "Um teste que verifica o comportamento futuro do sistema nas próximas versões"
    correct: 0
  - question: "Por que testes não garantem ausência de bugs?"
    options:
      - "Porque os testes são escritos por humanos, que cometem erros de digitação"
      - "Porque as ferramentas de teste ainda não são precisas o suficiente"
      - "Os testes só cobrem os cenários que você imaginou; um caso não previsto não é capturado"
      - "Porque os testes verificam só a interface, nunca a lógica de negócio"
    correct: 2
  - question: "O que significa dizer que testes são 'documentação executável'?"
    options:
      - "Que os testes geram arquivos de documentação em HTML automaticamente"
      - "Que você pode usar a suíte de testes no lugar de um README escrito à mão"
      - "Que os testes documentam a porcentagem de cobertura de código do projeto"
      - "Um teste bem nomeado descreve o comportamento esperado e falha quando ele muda"
    correct: 3
---
