...
quiz:
  - question: "Por que a pirâmide de testes recomenda mais testes unitários e poucos E2E?"
    options:
      - "Porque os testes E2E são, de modo geral, bem mais difíceis e trabalhosos de escrever do que os testes das outras camadas da pirâmide"
      ...
      - "Porque os frameworks de teste E2E mais conhecidos do mercado, como o Playwright, costumam ser ferramentas pagas e bastante caras"
      - "Porque os testes unitários, por sua própria natureza, acabam cobrindo uma quantidade muito maior de linhas de código do que os E2E"
    correct: 1
  - question: "O que diferencia um teste de integração de um teste unitário?"
    options:
      ...
      - "Os testes de integração utilizam sempre um banco de dados real durante a sua execução, enquanto os testes unitários usam apenas mocks"
      - "Os testes de integração são, de forma geral, perceptivelmente mais rápidos de executar do que os testes unitários do mesmo projeto"
      - "Os testes unitários servem apenas para testar funções soltas, enquanto os testes de integração servem apenas para testar componentes"
    correct: 0
  - question: "Qual é a principal vantagem do teste E2E sobre os demais?"
    options:
      - "É um tipo de teste perceptivelmente mais rápido de executar do que as outras camadas de teste que estão presentes na pirâmide de testes"
      - "É um tipo de teste bem mais fácil de manter ao longo do tempo, principalmente quando o código da aplicação muda com bastante frequência"
      ...
      - "É um tipo de teste que detecta os bugs de lógica do sistema com uma precisão bem maior do que a alcançada pelos testes unitários"
    correct: 2
  - question: "Qual teste seria mais adequado para verificar que 'filtrar artigos por tag retorna apenas artigos com aquela tag'?"
    options:
      - "Deveria ser um teste E2E, porque a funcionalidade de filtrar artigos por tag acaba envolvendo diretamente a interface do usuário na tela"
      - "Deveria ser um teste de integração, já que o filtro de artigos é considerado uma funcionalidade de negócio importante dentro da aplicação"
      - "Não seria necessário escrever nenhum teste para isso, porque o comportamento esperado dessa função de filtro já é totalmente óbvio por si só"
