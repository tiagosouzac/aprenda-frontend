...
quiz:
  - question: "Por que a pirâmide de testes recomenda mais testes unitários e poucos E2E?"
    options:
      - "Porque os testes E2E são bem mais difíceis e trabalhosos de escrever"
      - "Unitários são rápidos e baratos; E2E são lentos e frágeis. A pirâmide equilibra os dois"
      - "Porque frameworks E2E como o Playwright costumam ser pagos e caros"
      - "Porque os testes unitários cobrem muito mais linhas de código que os E2E"
    correct: 1
  - question: "O que diferencia um teste de integração de um teste unitário?"
    options:
      - "Unitário verifica uma peça isolada; integração verifica várias peças funcionando juntas"
      - "Os testes de integração usam um banco real e os unitários usam só mocks"
      - "Os testes de integração são mais rápidos de executar que os unitários"
      - "Unitários testam só funções soltas; integração testa só componentes"
    correct: 0
  - question: "Qual é a principal vantagem do teste E2E sobre os demais?"
    options:
      - "É um tipo de teste mais rápido de executar que as outras camadas da pirâmide"
      - "É um tipo de teste bem mais fácil de manter quando o código muda"
      - "Roda o navegador real com a app completa — verifica tudo como o usuário experimenta"
      - "Detecta bugs de lógica com mais precisão do que os testes unitários"
    correct: 2
  - question: "Qual teste seria mais adequado para verificar que 'filtrar artigos por tag retorna apenas artigos com aquela tag'?"
    options:
      - "E2E, porque a funcionalidade de filtrar por tag envolve a interface do usuário"
      - "Integração, já que o filtro é uma funcionalidade de negócio da aplicação"
      - "Nenhum: o comportamento dessa função de filtro já é totalmente óbvio"
      - "Unitário — a função de filtro é pura: entra array + tag, sai array filtrado, sem mocks"
    correct: 3
---
