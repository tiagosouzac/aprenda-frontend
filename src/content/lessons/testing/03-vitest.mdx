...
quiz:
  - question: "Por que Vitest precisa de pouca configuração em projetos Vite?"
    options:
      - "Porque Vitest não suporta TypeScript"
      ...
      - "Porque Vitest não precisa transpilar o código"
      - "Porque Vitest usa Node.js puro sem bundler"
    correct: 1
  - question: "Qual é a diferença entre .toBe() e .toEqual()?"
    options:
      ...
      - ".toBe() funciona apenas com strings; .toEqual() funciona com qualquer tipo"
    correct: 0
  - question: "O que o modo --watch do Vitest faz?"
    options:
      - "Monitora uso de memória durante os testes"
      - "Exibe os testes em uma janela gráfica"
      ...
      - "Grava o tempo de execução de cada teste"
    correct: 2
  - question: "Como você testa que uma função lança um erro?"
    options:
      - "Envolva a chamada em try/catch e use expect no catch"
      - "Use expect(funcaoQueLanca()).toBe(Error)"
      - "Não é possível testar erros lançados com Vitest"
