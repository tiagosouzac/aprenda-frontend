...
quiz:
  - question: "Por que Vitest precisa de pouca configuração em projetos Vite?"
    options:
      - "Porque o Vitest, ao contrário do que dizem, não dá suporte a TypeScript nos testes"
      - "Vitest reutiliza a config do Vite: os mesmos plugins, aliases e resolução de módulos"
      - "Porque o Vitest não precisa transpilar o código antes de rodar os testes"
      - "Porque o Vitest roda em Node.js puro, sem nenhum bundler por baixo"
    correct: 1
  - question: "Qual é a diferença entre .toBe() e .toEqual()?"
    options:
      - ".toBe() usa igualdade estrita (===); .toEqual() compara o conteúdo, certo para objetos e arrays"
      - "Não há diferença real: os dois matchers são intercambiáveis em qualquer caso"
      - "O .toEqual() é executado perceptivelmente mais rápido do que o .toBe()"
      - "O .toBe() só funciona com strings; o .toEqual(), com qualquer tipo de valor"
    correct: 0
  - question: "O que o modo --watch do Vitest faz?"
    options:
      - "Monitora o uso de memória durante os testes, para encontrar vazamentos"
      - "Exibe os resultados dos testes numa janela gráfica, não no terminal"
      - "Fica rodando e re-executa os testes afetados a cada arquivo salvo — feedback imediato"
      - "Grava o tempo de execução de cada teste num relatório de desempenho"
    correct: 2
  - question: "Como você testa que uma função lança um erro?"
    options:
      - "Envolver a chamada em try/catch e usar o expect dentro do bloco catch"
      - "Usar expect(funcaoQueLanca()).toBe(Error), comparando o retorno com Error"
      - "Não é possível testar erros lançados usando o Vitest, ele não suporta isso"
      - "expect(() => funcaoQueLanca()).toThrow() — passe a função como callback, não o resultado"
    correct: 3
---
