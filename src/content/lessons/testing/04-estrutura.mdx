...
quiz:
  - question: "O que representa cada etapa do padrão Arrange/Act/Assert?"
    options:
      - "Arrange é escrever o teste, Act é rodar no terminal e Assert é ler o resultado"
      - "Arrange prepara o cenário; Act executa a ação testada; Assert verifica o resultado"
      - "Arrange é importar os módulos, Act é chamar as funções e Assert é usar o expect"
      - "São três arquivos separados dentro de um projeto de testes, em pastas distintas"
    correct: 1
  - question: "Por que o nome do teste importa?"
    options:
      - "Um bom nome descreve o comportamento esperado, então você sabe na hora o que falhou"
      - "Porque nomes de teste mais longos deixam a suíte mais lenta de rodar"
      - "Porque o nome serve só para organizar visualmente os testes no terminal"
      - "Na verdade o nome não importa: o que importa em um teste é só o expect"
    correct: 0
  - question: "Para que serve o beforeEach?"
    options:
      - "Serve para executar todos os testes daquele arquivo em paralelo entre si"
      - "Serve para importar as dependências do teste só quando forem necessárias"
      - "Roda antes de cada teste no describe — ideal para limpar estado e preparar dados comuns"
      - "Serve para definir a ordem em que os testes do describe são executados"
    correct: 2
  - question: "Quais são os três tipos de caso que uma boa suite de testes deve cobrir?"
    options:
      - "Os testes rápidos, os de velocidade média e os testes lentos da suíte"
      - "Os testes unitários, os de integração e os de ponta a ponta (E2E)"
      - "Os testes síncronos, os assíncronos e os que verificam um componente"
      - "Caso feliz (entrada válida), casos limite (vazio, zero, máximo) e caso de erro"
    correct: 3
---
