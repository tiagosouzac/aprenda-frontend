...
quiz:
  - question: "Por que mockar fetch nos testes em vez de fazer requisições reais?"
    options:
      - "Porque a função fetch não consegue funcionar quando usada dentro do Vitest"
      - "Requisições reais são lentas, dependem da rede e tornam os testes instáveis"
      - "Porque o fetch é uma API exclusiva do navegador e não existe no Node.js"
      - "Porque os mocks são sempre mais rápidos de executar do que um fetch real"
    correct: 1
  - question: "Qual é a diferença entre vi.fn() e vi.mock()?"
    options:
      - "vi.fn() cria uma função mock individual; vi.mock() substitui um módulo inteiro"
      - "O vi.fn() é mais rápido nos testes e o vi.mock() é o mais preciso dos dois"
      - "O vi.fn() só funciona com funções assíncronas; o vi.mock(), com qualquer uma"
      - "Os dois são equivalentes: vi.fn() e vi.mock() fazem exatamente a mesma coisa"
    correct: 0
  - question: "O que significa 'testar o mock em vez do código'?"
    options:
      - "Quando o teste usa o expect sobre o mock em vez de sobre o componente testado"
      - "Quando você usa vi.mock() para substituir módulos de bibliotecas de terceiros"
      - "O mock substitui tanto código real que o teste só confirma que o mock foi chamado"
      - "Quando o mock que você escreveu fica mais complexo que o código que substitui"
    correct: 2
  - question: "Qual é a heurística correta sobre o que mockar?"
    options:
      - "Mockar absolutamente tudo o que a função toca, para isolar cada teste por completo"
      - "Mockar apenas as funções assíncronas, deixando as síncronas sem nenhum mock"
      - "Nunca mockar nada, usando sempre dados e dependências reais nos testes"
      - "Mockar dependências externas (rede, disco, tempo); não mockar a lógica interna da app"
    correct: 3
---
