...
quiz:
  - question: "Por que mockar fetch nos testes em vez de fazer requisições reais?"
    options:
      - "Porque fetch não funciona com Vitest"
      ...
      - "Porque fetch é uma API do browser e não existe no Node.js"
      - "Porque mocks são sempre mais rápidos que fetch"
    correct: 1
  - question: "Qual é a diferença entre vi.fn() e vi.mock()?"
    options:
      - "vi.fn() é mais rápido; vi.mock() é mais preciso"
      ...
      - "vi.fn() funciona apenas com funções assíncronas; vi.mock() funciona com qualquer tipo"
      - "São equivalentes — apenas sintaxes diferentes"
    correct: 0
  - question: "O que significa 'testar o mock em vez do código'?"
    options:
      - "Quando o teste usa expect no mock em vez do componente"
      - "Quando você usa vi.mock() para módulos de terceiros"
      ...
      - "Quando o mock é mais complexo que o código que ele substitui"
    correct: 2
  - question: "Qual é a heurística correta sobre o que mockar?"
    options:
      - "Mockar tudo para garantir isolamento total"
      - "Mockar apenas funções assíncronas"
      - "Nunca mockar — sempre usar dados reais"
