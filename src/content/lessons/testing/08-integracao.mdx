...
quiz:
  - question: "O que um teste de integração verifica que um teste unitário de componente não consegue?"
    options:
      - "A renderização visual do componente"
      ...
      - "Que a sintaxe do código está correta"
      - "Que as props estão tipadas corretamente com TypeScript"
    correct: 1
  - question: "Por que usar localStorage.clear() no beforeEach de testes que envolvem persistência?"
    options:
      ...
      - "Porque localStorage não funciona em jsdom sem ser limpo"
      - "Para que o TypeScript consiga tipar os dados do localStorage"
      - "Por convenção de estilo — não tem impacto técnico"
    correct: 0
  - question: "Quando um teste de integração compensa mais do que vários testes unitários separados?"
    options:
      - "Sempre — integração é sempre mais completo"
      - "Quando o componente tem mais de 100 linhas"
      ...
      - "Quando há mais de 5 estados diferentes"
    correct: 2
  - question: "Como testar que um componente exibe um estado de loading enquanto o fetch está pendente?"
    options:
      - "Não é possível testar estados transitórios com Testing Library"
      - "Usar setTimeout para esperar o fetch completar"
      - "Apenas verificar o estado final após o fetch completar"
