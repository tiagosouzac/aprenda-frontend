...
quiz:
  - question: "O que diferencia um teste E2E de um teste de integração com jsdom?"
    options:
      - "Os testes E2E são mais rápidos e bem mais precisos do que os de integração com jsdom"
      - "E2E roda um browser real com a app completa: fetch, render e navegação reais; jsdom só simula o DOM"
      - "Os testes E2E sempre usam mocks para tudo; os de integração com jsdom nunca usam"
      - "Os E2E verificam só a interface visual; os de integração, só a lógica em JavaScript"
    correct: 1
  - question: "Por que Playwright espera automaticamente antes de interagir com elementos?"
    options:
      - "Apps web têm estados assíncronos; o Playwright espera o elemento ficar pronto, sem sleeps manuais"
      - "É uma limitação técnica: o Playwright não consegue interagir sem esperar antes"
      - "É um comportamento criado para simular conexões de internet mais lentas"
      - "É um comportamento opcional, que pode ser ligado ou desligado por teste"
    correct: 0
  - question: "Por que getByRole é preferível a seletores CSS em testes E2E?"
    options:
      - "O getByRole é executado mais rápido dentro do browser do que um seletor CSS"
      - "Os seletores CSS não conseguem funcionar em conjunto com o Playwright"
      - "Seletores CSS quebram em refatoração; getByRole acha o botão pelo papel e nome acessível"
      - "O getByRole é só uma convenção da comunidade, sem impacto técnico real"
    correct: 2
  - question: "O que deve ser testado com E2E em vez de testes de integração?"
    options:
      - "Toda e qualquer lógica de negócio crítica da aplicação, sem nenhuma exceção"
      - "Todos os formulários e validações de campos de qualquer tela da aplicação"
      - "Todas as funções puras e cálculos, que costumam ser a base da lógica da app"
      - "Apenas os fluxos mais críticos do produto; E2E é lento e caro, integração cobre o resto"
    correct: 3
---
