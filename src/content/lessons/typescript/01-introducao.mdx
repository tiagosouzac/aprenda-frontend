...
quiz:
  - question: "O que acontece com os tipos TypeScript quando o código é compilado?"
    options:
      - "Eles são convertidos em comentários dentro do JavaScript gerado, servindo apenas como documentação para quem lê o código"
      - "Eles são convertidos em verificações de runtime com if e throw, que validam os tipos enquanto o programa está rodando"
      - "Eles permanecem como metadados acessíveis em tempo de execução, podendo ser lidos por reflexão durante a execução do código"
      - "Eles são apagados — o output é JavaScript puro, sem qualquer rastro dos tipos"
    correct: 3
  - question: "Por que TypeScript não detecta erros em dados que chegam de uma API?"
    options:
      - "Porque o TypeScript não oferece nenhum suporte ao uso da função fetch, então não consegue analisar requisições de rede"
      - "Porque a verificação de tipos ocorre em tempo de compilação — TypeScript analisa o código-fonte, não os dados que existem em runtime"
      - "Porque o TypeScript não tem suporte ao formato JSON e por isso acaba ignorando qualquer dado que chegue nesse formato"
      - "Porque toda e qualquer API sempre acaba retornando o tipo any, o que faz o TypeScript desistir de verificar os dados"
    correct: 1
  - question: "O que significa dizer que TypeScript é um superset de JavaScript?"
    options:
      - "Todo JavaScript válido é TypeScript válido — TS adiciona sintaxe nova (tipos) sem remover nada do JS"
      - "Significa que o TypeScript é executado de forma perceptivelmente mais rápida do que o JavaScript equivalente na prática"
      - "Significa que o TypeScript substitui completamente o JavaScript e passa a rodar direto dentro do navegador sem conversão"
      - "Significa que o TypeScript possui uma engine própria de execução, separada e independente do motor V8 dos navegadores"
    correct: 0
  - question: "Qual é o benefício mais prático de TypeScript no dia a dia?"
    options:
      - "O principal benefício é gerar um código final perceptivelmente menor em produção, reduzindo bastante o peso da aplicação"
      - "O principal benefício é garantir compatibilidade com navegadores bem mais antigos que já não recebem mais atualização"
      - "Autocompletar preciso no editor e erros destacados antes de rodar — o compilador sabe a forma exata de cada dado"
      - "O principal benefício é reduzir o tempo de download da página, já que os tipos deixam os arquivos finais mais enxutos"
    correct: 2
---
