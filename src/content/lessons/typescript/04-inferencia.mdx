...
quiz:
  - question: "Por que parâmetros de função precisam de anotação de tipo?"
    options:
      - "É só uma convenção de estilo da comunidade; anotar os parâmetros não é obrigatório no TS"
      - "Porque os parâmetros são o único lugar da linguagem onde a inferência de tipos não funciona"
      - "Porque o compilador exige uma anotação de tipo explícita em todos os lugares do código"
      - "O TypeScript não infere o tipo dos parâmetros: ao contrário de variáveis com valor inicial, eles só têm valor na chamada"
    correct: 3
  - question: "Quando faz sentido anotar o tipo de retorno de uma função mesmo que TypeScript possa inferi-lo?"
    options:
      - "Nunca: a inferência do TypeScript já é sempre suficiente para qualquer tipo de função"
      - "Quando a função é uma API pública ou o retorno é complexo — serve de documentação e pega divergências"
      - "Apenas quando a função é assíncrona e por isso acaba devolvendo uma Promise"
      - "Apenas nas funções que devolvem um objeto como resultado final da sua execução"
    correct: 1
  - question: "O que acontece quando TypeScript infere o tipo de um elemento de array em map()?"
    options:
      - "O TS usa o tipo do array para inferir o do callback: em artigos.map(a => …), a é Artigo se artigos for Artigo[]"
      - "O TypeScript sempre infere any para o callback, então a anotação manual é sempre necessária"
      - "O TypeScript exige anotar o tipo do parâmetro do callback, pois não o infere a partir do array"
      - "O TypeScript infere o tipo do parâmetro a partir do valor que o próprio callback retorna"
    correct: 0
  - question: "O que é widening de tipo?"
    options:
      - "Quando o TypeScript estreita um tipo amplo para um mais específico após um typeof ou if"
      - "Quando o TypeScript converte automaticamente um valor do tipo number para o tipo any"
      - "Quando o TS infere 'string' (não a literal) para um let, porque ele permite reatribuição depois"
      - "Quando o TypeScript adiciona o valor null como mais uma possibilidade de um tipo já existente"
    correct: 2
---
