...
quiz:
  - question: "Qual é a diferença entre um parâmetro opcional (x?: string) e um com valor padrão (x: string = 'padrão')?"
    options:
      - "Não há diferença prática: um parâmetro opcional e um com valor padrão funcionam igual"
      - "O opcional serve só para valores string, e o com valor padrão serve só para numbers"
      - "O parâmetro com valor padrão não pode, de forma alguma, ser omitido na chamada da função"
      - "x?: string é string | undefined e pode ser omitido; x = 'padrão' também, mas dentro da função é sempre string"
    correct: 3
  - question: "O que é um tipo de função em TypeScript?"
    options:
      - "É simplesmente o tipo do valor que a função devolve, ou seja, o seu tipo de retorno"
      - "Uma descrição do formato de uma função (parâmetros e retorno), para tipar variáveis que guardam funções"
      - "Uma função especial que recebe outros tipos como argumento e devolve um novo tipo"
      - "Uma função declarada com a palavra-chave type no lugar da palavra-chave function"
    correct: 1
  - question: "Quando o retorno de uma função é never em vez de void?"
    options:
      - "A função NUNCA completa (lança erro ou loop infinito); void é para quem retorna undefined, never para quem não retorna"
      - "Quando a função não devolve nenhum valor que seja de fato útil para quem a chamou"
      - "Quando a função retorna explicitamente null como resultado padrão ao terminar a execução"
      - "never e void são totalmente intercambiáveis, então tanto faz qual dos dois você usa"
    correct: 0
  - question: "Por que desestruturar os parâmetros de uma função e anotar com uma interface é preferível a listar cada parâmetro individualmente?"
    options:
      - "Porque as interfaces são compiladas mais rápido do que listar cada parâmetro um a um"
      - "Porque o TypeScript não dá suporte a funções que recebem mais de quatro parâmetros declarados"
      - "Com muitos campos do mesmo objeto, a interface é mais legível e reutilizável em outros lugares"
      - "Porque a desestruturação dos parâmetros é obrigatória quando o strict mode está ativado no projeto"
    correct: 2
---
