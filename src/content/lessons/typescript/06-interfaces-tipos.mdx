...
quiz:
  - question: "Qual é a principal diferença prática entre interface e type para descrever objetos?"
    options:
      - "A interface é compilada mais rápido pelo TypeScript do que um type equivalente"
      - "O type só pode descrever primitivos como string e number, nunca a forma de um objeto"
      - "Não há diferença alguma: interface e type são completamente intercambiáveis em tudo"
      - "interface usa extends e declaration merging (aberta); type é melhor para unions e mapped types, mas não reabre"
    correct: 3
  - question: "O que é declaração merging em interfaces?"
    options:
      - "Combinar dois arquivos TypeScript diferentes em um só durante a etapa de compilação"
      - "Declarar a mesma interface em dois lugares e o TS junta as propriedades — útil para estender libs"
      - "Exportar a mesma interface a partir de dois módulos, e o TypeScript junta as duas exportações"
      - "Criar uma interface nova que herda todas as propriedades de outra que já existia antes"
    correct: 1
  - question: "Quando usar type em vez de interface?"
    options:
      - "Para unions (A | B), intersections (A & B), mapped types e aliases de primitivos, ou para impedir reabertura"
      - "Deve-se usar type sempre, porque ele é superior à interface em todos os aspectos da linguagem"
      - "Deve-se usar type apenas para descrever valores de tipos primitivos soltos, como string ou number"
      - "Deve-se usar type só dentro de arquivos de declaração .d.ts, nunca em código normal"
    correct: 0
  - question: "Como readonly difere de const para propriedades de objetos?"
    options:
      - "As palavras readonly e const são totalmente equivalentes para propriedades de um objeto"
      - "O readonly é usado para arrays, e o const para objetos comuns dentro da linguagem"
      - "const impede reatribuir a variável; readonly, a propriedade — dá para ter const artigo com readonly id"
      - "O readonly só funciona em propriedades de classe, e não em objetos literais comuns do código"
    correct: 2
---
