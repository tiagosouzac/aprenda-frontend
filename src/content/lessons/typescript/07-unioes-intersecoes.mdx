...
quiz:
  - question: "Por que TypeScript exige narrowing antes de usar uma propriedade de um union type?"
    options:
      - "É exigido apenas por uma convenção de estilo recomendada pela comunidade, mas o narrowing não é de fato obrigatório no TS"
      - "Porque uma união de tipos simplesmente não dá suporte ao acesso de nenhuma propriedade enquanto ela não for resolvida antes"
      - "Porque o narrowing dos tipos só é realmente necessário quando o valor do union está sendo usado dentro de uma função"
  ...
  - question: "O que é uma discriminated union?"
    options:
      - "É simplesmente uma união que combina mais de dois tipos diferentes ao mesmo tempo dentro de uma mesma declaração no TypeScript"
      ...
      - "É uma união em que o próprio compilador do TypeScript escolhe e discrimina os tipos automaticamente, sem você precisar fazer nada"
      - "É um tipo de união especial que tem a garantia de nunca poder assumir o valor null em nenhum momento da execução do programa"
    correct: 1
  - question: "Qual é o resultado de string & number em uma intersection?"
    options:
      ...
      - "string — nesse caso o tipo considerado mais amplo entre os dois é o que acaba vencendo o conflito da intersection no final"
      - "number — nesse caso o tipo considerado mais específico entre os dois é o que acaba prevalecendo dentro da intersection"
      - "any — o TypeScript não consegue resolver o conflito entre os dois tipos e por isso acaba caindo no tipo any por segurança"
    correct: 0
  - question: "O que o type guard 'titulo' in obj verifica?"
    options:
      - "Verifica se o valor que está guardado na propriedade obj.titulo é especificamente uma string, e não outro tipo de dado"
      - "Verifica se a propriedade titulo do objeto foi declarada como readonly, ou seja, se ela pode ou não ser reatribuída depois"
      ...
      - "Verifica se a propriedade titulo do objeto é uma propriedade opcional, ou seja, se ela pode estar ausente daquele objeto"
    correct: 2
---
