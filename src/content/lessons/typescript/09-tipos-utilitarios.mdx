...
quiz:
  - question: "Qual é a diferença entre Partial<T> e Pick<T, K>?"
    options:
      - "São equivalentes na prática: Partial<T> e Pick<T, K> produzem exatamente o mesmo tipo final"
      - "O Partial só pode ser aplicado a interfaces, enquanto o Pick só pode ser aplicado a type aliases"
      - "O Pick remove do tipo as propriedades indicadas; o Partial transforma todas elas em readonly"
      - "Partial<T> torna todas as propriedades de T opcionais; Pick<T, K> seleciona apenas as propriedades K de T"
    correct: 3
  - question: "Quando usar Record<K, V>?"
    options:
      - "Sempre que quiser substituir um array por uma estrutura de dados um pouco mais organizada"
      - "Quando você precisa de um objeto/mapa com chaves do tipo K e valores do tipo V — útil quando K é uma union de literais"
      - "Principalmente quando precisa criar objetos aninhados, com vários níveis de propriedades"
      - "É apenas uma forma bem mais verbosa de escrever a assinatura de índice { [key: string]: V }"
    correct: 1
  - question: "O que Awaited<Promise<Artigo[]>> resulta?"
    options:
      - "Artigo[] — o Awaited extrai o tipo do valor resolvido de uma Promise"
      - "Promise<Artigo[]> — o Awaited só confirma o tipo da Promise, sem extrair o valor de dentro dela"
      - "Apenas Artigo, no singular — o Awaited extrai o valor da Promise e ainda desempacota o array"
      - "never — o Awaited não consegue desempacotar uma Promise que contém um array de objetos"
    correct: 0
  - question: "Por que Omit<Artigo, 'id' | 'slug'> é útil para formulários de criação?"
    options:
      - "Porque os formulários, de forma geral, não dão suporte a campos que sejam do tipo id"
      - "Porque o Omit também remove automaticamente todas as propriedades marcadas como readonly"
      - "Ao criar um artigo, o servidor gera id e slug; o Omit cria o tipo certo sem duplicar a interface"
      - "Porque o uso do Omit passou a ser obrigatório para tipar qualquer formulário de criação no TS"
    correct: 2
---
