...
quiz:
  - question: "O que um mapped type faz?"
    options:
      - "Transforma cada item de um array"
      - "Mapeia strings para números"
      - "Cria um Record automaticamente"
  ...
  - question: "O que `type Evento = `on${string}`` representa?"
    options:
      ...
      - "A string literal 'onstring'"
      - "Uma concatenação de strings em runtime"
      - "Um tipo de array"
    correct: 0
  - question: "Quando faz sentido usar tipos avançados (mapped, conditional, template literal)?"
    options:
      - "Sempre que possível — mais tipos é melhor"
      - "Apenas em projetos grandes"
      ...
      - "Apenas quando strict: true está ativado"
    correct: 2
---
