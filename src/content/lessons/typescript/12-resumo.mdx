...
quiz:
  - question: "Por que strict: true é a primeira configuração a ativar em um projeto novo?"
    options:
      - "Porque acelera a compilação"
      - "Porque é obrigatório para usar generics"
      - "Porque é necessário para usar interfaces"
  ...
  - question: "Qual é a relação entre interface Artigo e as funções que recebem Artigo como parâmetro?"
    options:
      - "Funções que recebem Artigo precisam ser declaradas dentro da interface"
      ...
      - "Funções e interfaces são independentes — não há relação"
      - "A interface precisa ser importada na mesma linha da função"
    correct: 1
  - question: "Quando você deve usar unknown em vez de any?"
    options:
      ...
      - "unknown é uma versão mais lenta de any"
      - "unknown é apenas para erros no catch"
      - "any e unknown são equivalentes com strict: true"
    correct: 0
  - question: "O que significa 'anote nas bordas, infira no meio'?"
    options:
      - "Anotar tipos nas primeiras e últimas linhas de cada arquivo"
      - "Anotar apenas em interfaces, nunca em funções"
      ...
      - "Anotar apenas parâmetros, nunca retornos"
    correct: 2
---
